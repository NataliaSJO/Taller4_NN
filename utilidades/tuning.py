...
CB_KEYS = {"factor", "rlr_patience", "min_delta", "min_lr"}
...
def _build_model_from_modelos(model_fn_name, cfg, X_train, y_train):
    ...
    # Permite tunear lr sin editar modelos.py (recompile en runtime)
    if "lr" in cfg:
        model.compile(
...
def _train_one(model_fn_name, cfg, X_train, y_train, X_val, y_val):
    ...
    return best_val, best_idx + 1, hist, model
...
def ofat_search(model_fn_name, base_cfg, search_steps, X_train, y_train, X_val, y_val):
    ...
    valid = ctor_tuneables | FIT_KEYS | CB_KEYS | {"lr", "seed"}

    trials = []
    best_cfg = base_cfg.copy()

    best_val, best_epoch, best_hist, best_model = _train_one(
        model_fn_name, best_cfg, X_train, y_train, X_val, y_val
    )
    trials.append({"param": "baseline", "value": "-", "best_val_loss": best_val, "best_epoch": best_epoch, **best_cfg})
    ...
    for param, values in search_steps:
        ...
        local_best = (best_val, best_epoch, best_hist, best_model, best_cfg.copy())

        for v in values:
            cfg = best_cfg.copy()
            cfg[param] = v

            val, ep, hist, model = _train_one(
                model_fn_name, cfg, X_train, y_train, X_val, y_val
            )

            trials.append({"param": param, "value": v, "best_val_loss": val, "best_epoch": ep, **cfg})

            if val < local_best[0]:
                local_best = (val, ep, hist, model, cfg.copy())

        best_val, best_epoch, best_hist, best_model, best_cfg = local_best
        print(f"{param:12} -> mejor: {best_cfg[param]} | val_loss={best_val:.6f} (época {best_epoch})")

    df_trials = pd.DataFrame(trials).sort_values("best_val_loss").reset_index(drop=True)
...
def successive_halving_search(model_fn_name, base_cfg, search_steps, X_train, y_train,
    ...
    all_combos = list(product(*values_grid))
    ...
    for i, combo in enumerate(combos, start=1):
        ...
        if progress_every and (i == 1 or i % progress_every == 0 or i == n_candidates):
            best_short = min(row["best_val_loss"] for row in short_trials)
            print(
                f"{model_fn_name}: {i}/{n_candidates} candidatos cortos | "
                f"mejor val_loss={best_short:.6f}"
            )

    df_short = pd.DataFrame(short_trials).sort_values("best_val_loss").reset_index(drop=True)
    ...
    for i, row in enumerate(finalists, start=1):
        ...
        print(f"final {i}/{len(finalists)} | mejor val_loss={best_val:.6f}")

    df_final = pd.DataFrame(trials).sort_values("best_val_loss").reset_index(drop=True)
